...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // Xâu chuỗi nhiều Function
        Function<String,String[]> f0 = uCase
                .andThen(s -> s.concat(" Buchalka"))
                .andThen(s -> s.split(" "));
        System.out.println(Arrays.toString(f0.apply(name)));
        //Xử lý phức tạp hơn với andThen()
        Function<String,String> f1 = uCase
                .andThen(s -> s.concat(" Buchalka"))
                .andThen(s -> s.split(" "))
                .andThen(s -> s[1].toUpperCase() + ", " + s[0]);
        System.out.println(f1.apply(name));
        //Đếm số ký tự sau khi xử lý chuỗi
        Function<String,Integer> f2 = uCase
                .andThen(s -> s.concat(" Buchalka"))
                .andThen(s -> s.split(" "))
                .andThen(s -> String.join(", ", s))
                //Gộp lại thành "TIM,
                // BUCHALKA" rồi tính độ dài chuỗi.
                .andThen(String::length);
        System.out.println(f2.apply(name));
        // in ra ký tự đầu tiên của mỗi tên
        ...
        Arrays.asList(names).forEach(s0
                ...
                .andThen(s1));
        //Predicate<T> để kiểm tra điều kiện
